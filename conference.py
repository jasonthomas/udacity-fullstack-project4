...
FEATURED_SPEAKER_KEY = "FEATURED_SPEAKER_"
...
SESS_GET_TYPE_CONF_REQUEST = endpoints.ResourceContainer(
    message_types.VoidMessage,
...
SESS_GET_SPEAKER_TYPE_REQUEST = endpoints.ResourceContainer(
    message_types.VoidMessage,
    speaker=messages.StringField(1, required=True),
    typeOfSession=messages.StringField(2, required=True),
)
...
# - - - Session objects - - - - - - - - - - - - - - - - -
    ...
    def _createSessionObject(self, request):
        ...
        # Get the conference object from the websafekey input
        try:
            wsck = request.wsck
            conf = ndb.Key(urlsafe=wsck).get()
        except:
        ...
        # Only user that created the conference can create sessions
        if user_id != conf.organizerUserId:
            raise endpoints.UnauthorizedException('Unauthorized! You are not '
        ...
        Session(**data).put()
        taskqueue.add(params={'wsck': wsck,
    ...
    def getConferenceSessions(self, request):
        ...
        if not sessions:
            raise endpoints.NotFoundException(
                'No sessions found with conference key: %s' % wsck)
        return SessionForms(
            items=[self._copySessionToForm(sess) for sess in
                   sessions]
        )

    @endpoints.method(SESS_GET_TYPE_CONF_REQUEST, SessionForms,
                      path='getConferenceSessionsByType',
                      http_method='GET', name='getConferenceSessionsByType')
    def getConferenceSessionsByType(self, request):
        """Return sessions by type for a particular conference."""
        wsck = request.websafeConferenceKey
        s_type = request.typeOfSession

        # create ancestor query for all key matches for this conference
        sessions = Session.query(ancestor=ndb.Key(urlsafe=wsck))
        sessions = sessions.filter(Session.typeOfSession == s_type)
        if not sessions:
            raise endpoints.NotFoundException(
                'No sessions found with session type: %s' % s_type)
        return SessionForms(
            items=[self._copySessionToForm(sess) for sess in
                   sessions]
        )
    ...
    def getSessionsBySpeaker(self, request):
        ...
        s_speaker = request.speaker

        sessions = Session.query()
        sessions = sessions.filter(Session.speaker == s_speaker)
        if not sessions:
            raise endpoints.NotFoundException(
                'No speakers found in sessions: %s' % s_speaker)
        return SessionForms(
            items=[self._copySessionToForm(sess) for sess in
                   sessions]
        )
...
# - - - Wishlist objects - - - - - - - - - - - - - - - - -
    def _copyWishlistToForm(self, wishlist):
        ...
        wlf = WishlistForm()
        for field in wlf.all_fields():
            if hasattr(wishlist, field.name):
                setattr(wlf, field.name, getattr(wishlist, field.name))
    ...
    def _addSessionToWishlist(self, request):
        ...
        # if wishlist exists, combine current sessionKeys with new sessionKeys
        # make sure they are unique and no duplicates using set()
        if wl:
            combinedKeys = list(set(wl.sessionKeys + sessionKeys))
            setattr(wl, 'sessionKeys', combinedKeys)
            wl.put()
    ...
    @endpoints.method(WishlistForm, WishlistForm, path='addSessionToWishlist',
    ...
    def _deleteSessionToWishlist(self, request):
        ...
        # if wishlist exists, remove keys
        if wl:
            newKeys = [x for x in wl.sessionKeys if x not in sessionKeys]
            setattr(wl, 'sessionKeys', newKeys)
            wl.put()
            return self._copyWishlistToForm(wl)
        else:
    ...
    def deleteSessionToWishlist(self, request):
        ...
        return self._deleteSessionToWishlist(request)
...
# - - - Custom Queries  - - - - - - - - - - - - - - - - -
    @endpoints.method(SESS_GET_TYPE_REQUEST, SessionForms,
                      path='getSessionsByTypeToday',
                      http_method='GET', name='getSessionsByTypeToday')
    def getSessionsByTypeToday(self, request):
        """Return all sessions of type for all Conferences with
    ...
    @endpoints.method(SESS_GET_SPEAKER_TYPE_REQUEST, SessionForms,
                      path='getSessionsBySpeakerType',
                      http_method='GET', name='getSessionsBySpeakerType')
    def getSessionsBySpeakerType(self, request):
        """Return all sessions given by this particular speaker
           and type, across all conferences"""
...
# - - - Featured Speaker - - - - - - - - - - - - - - - - -
    ...
    def getFeaturedSpeaker(self, request):
        """If speaker is in multiple sessions at the conference,
        make speaker featured speaker and store information in memcache. """

        key = FEATURED_SPEAKER_KEY + request.websafeConferenceKey
        return StringMessage(data=memcache.get(key) or "No featured speaker")

    @staticmethod
    def _cacheFeaturedSpeaker(speaker, wsck):
        # get all sessions for speaker and wsck
        sessions = Session.query(ndb.AND(Session.speaker == speaker,
                                         Session.wsck == wsck)).fetch()
        # set cache key if sessions greater than 1
        if len(sessions) > 1:
            key = FEATURED_SPEAKER_KEY + wsck
